...
//Sampling
const int NUM_SAMPLES_LOW=7, NUM_SAMPLES_MED=15, NUM_SAMPLES_HIGH=30;
...
int follow_me(int s, int m) {
  ...
  if (s == S_CENTER_LEFT) { step(degree_to_steps(BASE_DEGREE),DIR_RIGHT,SPD_VSLOW); s=S_CENTER;}
  if (s == S_CENTER_RIGHT) { step(degree_to_steps(BASE_DEGREE),DIR_LEFT,SPD_VSLOW); s=S_CENTER;}
...
int stare_at_me(int s, int p) {
  ...
  //TODO: Simplify
  if (s == S_LEFT) {
    if (p == POS_LEFT) {s_o=S_LEFT;}
    if (p == POS_CENTER_LEFT) {step(degree_to_steps(BASE_DEGREE),DIR_RIGHT,SPD_VSLOW);s_o=S_CENTER_LEFT;}
    if (p == POS_CENTER) {step(degree_to_steps(2*BASE_DEGREE),DIR_RIGHT,SPD_VSLOW);s_o=S_CENTER;}
    if (p == POS_CENTER_RIGHT) {step(degree_to_steps(3*BASE_DEGREE),DIR_RIGHT,SPD_VSLOW);s_o=S_CENTER_RIGHT;}
    if (p == POS_RIGHT) {step(degree_to_steps(4*BASE_DEGREE),DIR_RIGHT,SPD_VSLOW),s_o=S_RIGHT;}
  }

  if (s == S_CENTER_LEFT) {
    if (p == POS_LEFT) {step(degree_to_steps(BASE_DEGREE),DIR_LEFT,SPD_VSLOW), s_o=S_LEFT;}
    if (p == POS_CENTER_LEFT) {s_o=S_CENTER_LEFT;}
    if (p == POS_CENTER) {step(degree_to_steps(BASE_DEGREE),DIR_RIGHT,SPD_VSLOW);s_o=S_CENTER;}
    if (p == POS_CENTER_RIGHT) {step(degree_to_steps(2*BASE_DEGREE),DIR_RIGHT,SPD_VSLOW);s_o=S_CENTER_RIGHT;}
    if (p == POS_RIGHT) {step(degree_to_steps(3*BASE_DEGREE),DIR_RIGHT,SPD_VSLOW);s_o=S_RIGHT;}
  }

  if (s == S_CENTER) {
    if (p == POS_LEFT) {step(degree_to_steps(2*BASE_DEGREE),DIR_LEFT,SPD_VSLOW),s_o=S_LEFT;}
    if (p == POS_CENTER_LEFT) {step(degree_to_steps(BASE_DEGREE),DIR_LEFT,SPD_VSLOW);s_o=S_CENTER_LEFT;}
    if (p == POS_CENTER) {s_o=S_CENTER;}
    if (p == POS_CENTER_RIGHT) {step(degree_to_steps(BASE_DEGREE),DIR_RIGHT,SPD_VSLOW);s_o=S_CENTER_RIGHT;}
    if (p == POS_RIGHT) {step(degree_to_steps(2*BASE_DEGREE),DIR_RIGHT,SPD_VSLOW);s_o=S_RIGHT;}
  }

  if (s == S_CENTER_RIGHT) {
    if (p == POS_LEFT) {step(degree_to_steps(3*BASE_DEGREE),DIR_LEFT,SPD_VSLOW); s_o=S_LEFT;}
    if (p == POS_CENTER_LEFT) {step(degree_to_steps(2*BASE_DEGREE),DIR_LEFT,SPD_VSLOW);s_o=S_CENTER_LEFT;}
    if (p == POS_CENTER) {step(degree_to_steps(BASE_DEGREE),DIR_LEFT,SPD_VSLOW);s_o=S_CENTER;}
    if (p == POS_CENTER_RIGHT) {s_o=S_CENTER_RIGHT;}
    if (p == POS_RIGHT) {step(degree_to_steps(BASE_DEGREE),DIR_RIGHT,SPD_VSLOW);s_o=S_RIGHT;}
  }

  if (s == S_RIGHT) {
    if (p == POS_LEFT) {step(degree_to_steps(4*BASE_DEGREE),DIR_LEFT,SPD_VSLOW);s_o=S_LEFT;}
    if (p == POS_CENTER_LEFT) {step(degree_to_steps(3*BASE_DEGREE),DIR_LEFT,SPD_VSLOW);s_o=S_CENTER_LEFT;}
    if (p == POS_CENTER) {step(degree_to_steps(2*BASE_DEGREE),DIR_LEFT,SPD_VSLOW);s_o=S_CENTER;}
    if (p == POS_CENTER_RIGHT) {step(degree_to_steps(BASE_DEGREE),DIR_LEFT,SPD_VSLOW);s_o=S_CENTER_RIGHT;}
    if (p == POS_RIGHT) {s_o=S_RIGHT;}
  }
